...
[MCP](https://modelcontextprotocol.io) server exposing the SerialifColor SDK as
two agent tools — `serialif-color_list` and `serialif-color_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/serialif-color-mcp)
make build

# 2. Provide credentials via the environment
export SERIALIF_COLOR_APIKEY=sk_live_xxx
...
```jsonc
// serialif-color_list: first page of records
{ "entity": "get_color_by_path" }
{ "entity": "get_color_by_path", "query": { } }

// serialif-color_load: one record by id
{ "entity": "get_color_by_path", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/serialif-color-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user serialif-color \
     -- "$PWD"/dist/*/serialif-color-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `serialif-color_list` and `serialif-color_load` tools now appear
   in new sessions. Ask the agent to *"list get_color_by_path using serialif-color"*
   and it calls `serialif-color_list` with `{"entity":"get_color_by_path"}`.
...
```sh
export SERIALIF_COLOR_APIKEY=sk_live_xxx            # API key
export SERIALIF_COLOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `serialif-color_list` tool
...
### Call the `serialif-color_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `serialif-color_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `serialif-color_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SERIALIF_COLOR_APIKEY` | API key sent with every request. |
| `SERIALIF_COLOR_BASE` | Optional override of the API base URL. |
