...
import (
	...
	omni "github.com/voxgig-sdk/serialif-color-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SerialifColorError); is {
				out := map[string]any{
					"name":    "SerialifColorError",
...
func omniCtx(arg any, client *sdk.SerialifColorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SerialifColorSDK); is {
			client = live
