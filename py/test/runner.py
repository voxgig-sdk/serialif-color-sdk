...
class SerialifColorTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SerialifColorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SerialifColorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SerialifColorTestRunner.getenv("SERIALIF_COLOR_TEST_LIVE")
        override = SerialifColorTestRunner.getenv("SERIALIF_COLOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SerialifColorTestRunner.getenv(key)
        ...
        explain = SerialifColorTestRunner.getenv("SERIALIF_COLOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SerialifColorTestRunner._test_control is not None:
            return SerialifColorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SerialifColorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SerialifColorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SerialifColorTestRunner.load_test_control()
...
def load_env_local():
    SerialifColorTestRunner.load_env_local()


def env_override(m):
    return SerialifColorTestRunner.env_override(m)


def entity_data(v):
    return SerialifColorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SerialifColorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SerialifColorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SerialifColorTestRunner.load_test_control()


def live_client_options():
    return SerialifColorTestRunner.live_client_options()


def live_delay_ms():
    return SerialifColorTestRunner.live_delay_ms()
