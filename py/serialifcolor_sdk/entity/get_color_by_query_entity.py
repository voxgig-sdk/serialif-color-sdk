...
class GetColorByQueryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetColorByQueryEntity(self._client, opts)
    ...
    def data_get(self) -> GetColorByQuery:
    ...
    def match_get(self) -> GetColorByQuery:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetColorByQuery:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetColorByQuery().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
