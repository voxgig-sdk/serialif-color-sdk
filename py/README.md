...
The Python SDK for the SerialifColor API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetColorByPath(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.SerialifColor(None).load(
    {"id": "test01"}, None
)
