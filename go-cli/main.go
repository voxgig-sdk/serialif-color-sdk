// Command SerialifColor-cli is a CLI + REPL for the SerialifColor SDK,
...
const prompt = "serialif-color"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_color_by_path get_color_by_query"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SERIALIF_COLOR_APIKEY (api key), SERIALIF_COLOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SERIALIF_COLOR_APIKEY carries the API key and
	// SERIALIF_COLOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SERIALIF_COLOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SERIALIF_COLOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSerialifColorSDK(opts)
