...
boru-driven command-line client **and** interactive REPL for the SerialifColor
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/serialif-color-cli)
make build

# 2. See usage (words, entities, env vars)
./serialif-color-cli --help

# 3. Provide credentials once, via the environment
export SERIALIF_COLOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./serialif-color-cli load 1 get_color_by_path            # {id:1} shorthand
./serialif-color-cli load '{id:1}' get_color_by_path       # explicit match map

# 5. Override the API base URL for a single call
SERIALIF_COLOR_BASE=https://api.example.com ./serialif-color-cli load 1 get_color_by_path

# 6. No arguments -> interactive REPL
./serialif-color-cli
serialif-color> load 1 get_color_by_path
serialif-color> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/serialif-color-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/serialif-color-cli load 1 get_color_by_path
   ```
...
```sh
./serialif-color-cli load 1 get_color_by_path          # scalar shorthand for {id:1}
./serialif-color-cli load '{id:1}' get_color_by_path     # explicit match map
```
...
./serialif-color-cli load 1 get_color_by_path
```
...
Run with no arguments to open a REPL (prompt `serialif-color>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_color_by_path`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/serialif-color-cli`. |
...
(`./serialif-color-cli <expr>`) and interactively (the REPL), and expressions compose the
