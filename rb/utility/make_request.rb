...
module SerialifColorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SerialifColorResponse.new({})
    ...
    if fetch_err
      ...
      response = SerialifColorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SerialifColorResponse.new(fetched)
    else
