...
class GetColorByQueryEntity
  ...
  def make
    opts = @_entopts.dup
    GetColorByQueryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SerialifColorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetColorByQuery, Hash] the current GetColorByQuery data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SerialifColorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetColorByQuery fields)
  def match_get
  ...
  #
  # @param reqmatch [GetColorByQueryLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetColorByQuery.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetColorByQuery, Hash] the loaded GetColorByQuery; raises SerialifColorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SerialifColorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
