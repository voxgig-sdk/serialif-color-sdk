...
The Ruby SDK for the SerialifColor API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GetColorByPath(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.SerialifColor(nil).load(
  { "id" => "test01" }, nil
)
