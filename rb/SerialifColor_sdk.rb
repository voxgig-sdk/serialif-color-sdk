...
require_relative 'SerialifColor_types'
...
class SerialifColorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SerialifColorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SerialifColorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SerialifColorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SerialifColorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SerialifColorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SerialifColorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SerialifColorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SerialifColorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SerialifColorError.new(
        "graphql_error", "SerialifColorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SerialifColorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
