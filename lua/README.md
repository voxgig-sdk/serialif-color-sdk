# SerialifColor Lua SDK



The Lua SDK for the SerialifColor API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GetColorByPath()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/serialif-color-sdk/releases)),
...
| `GetColorByPath` | `(data) -> GetColorByPathEntity` | Create a GetColorByPath entity instance. |
| `GetColorByQuery` | `(data) -> GetColorByQueryEntity` | Create a GetColorByQuery entity instance. |
...
| --- | --- | --- |
| `base` | `table` | Requested base color |
| `base_without_alpha` | `table` | Base color without alpha channel |
| `base_without_alpha_contrasted_text` | `table` | Black or white text color that contrasts with base color |
| `complementary` | `table` | Complementary color |
| `complementary_without_alpha` | `table` | Complementary color without alpha channel |
| `complementary_without_alpha_contrasted_text` | `table` | Black or white text color that contrasts with complementary color |
...
```lua
local get_color_by_query, err = client:GetColorByQuery():load()
```
...
```
lua/
├── serialif-color_sdk.lua    -- Main SDK module
...
The main module (`serialif-color_sdk`) exports the SDK constructor
