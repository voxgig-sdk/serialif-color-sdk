...
The Lua SDK for the SerialifColor API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetColorByPath(nil):load({ id = "example_id" }, nil)
...
local result, err = client:SerialifColor(nil):load(
  { id = "test01" }, nil
)
