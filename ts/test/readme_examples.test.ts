...
//     (\`new SerialifColorSDK(...)\` / \`SerialifColorSDK.test(...)\` become
//     \`SerialifColorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SerialifColorSDK'
...
const TEST_SEED = {"entity":{"get_color_by_path":{"test01":{"id":"test01"}},"get_color_by_query":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SerialifColorSDK(...)\` and any
// \`SerialifColorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SerialifColorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SerialifColorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
