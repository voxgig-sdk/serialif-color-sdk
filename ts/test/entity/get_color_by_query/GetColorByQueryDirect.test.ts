...
describe('GetColorByQueryDirect', async () => {
  ...
  test('direct-load-get_color_by_query', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_color_by_query', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SERIALIF_COLOR_TEST_GET_COLOR_BY_QUERY_ENTID']
