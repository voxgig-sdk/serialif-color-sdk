# SerialifColor TypeScript SDK



The TypeScript SDK for the SerialifColor API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.GetColorByPath()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `SerialifColorSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `SerialifColorSDK.test(testopts?, sdkopts?)` | `SerialifColorSDK` | Create a test-mode client. |
...
| `client` | `client(): SerialifColorSDK` | Return the parent SDK client. |
...
```ts
const get_color_by_query = await client.GetColorByQuery().load()
```
...
serialif-color/
├── src/
│   ├── SerialifColorSDK.ts        # Main SDK class
