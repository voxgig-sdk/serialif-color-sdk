...
// SerialifColor SDK feature test
//
...
class FtCtrl extends SerialifColorControl
{
...
class FtClient
{
    ...
    private SerialifColorUtility $utility;

    public function __construct(SerialifColorUtility $utility, array $options)
    {
    ...
    public function get_utility(): SerialifColorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SerialifColorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SerialifColorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SerialifColorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SerialifColorConfig::make_feature($name);
    ...
    private static function buildUrl(SerialifColorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SerialifColorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SerialifColorResponse($fetched);
        }
    ...
    private function fail(SerialifColorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SerialifColorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
