...
// SerialifColor SDK retry feature
...
class SerialifColorRetryFeature extends SerialifColorBaseFeature
{
    ...
    public function init(SerialifColorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SerialifColorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SerialifColorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SerialifColorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
