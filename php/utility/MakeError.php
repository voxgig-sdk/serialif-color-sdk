...
class SerialifColorMakeError
{
    public static function call(?SerialifColorContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new SerialifColorContext([], null);
        }
        $op = $ctx->op ?? new SerialifColorOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new SerialifColorResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof SerialifColorError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new SerialifColorError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof SerialifColorError) {
